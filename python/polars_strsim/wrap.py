import polars_strsim.polars_strsim as rust

def levenshtein(df, col_a, col_b, name='levenshtein'):
    return rust.levenshtein(df, col_a, col_b, name)

def jaro(df, col_a, col_b, name='jaro'):
    return rust.jaro(df, col_a, col_b, name)

def jaro_winkler(df, col_a, col_b, name='jaro_winkler'):
    return rust.jaro_winkler(df, col_a, col_b, name)

def jaccard(df, col_a, col_b, name='jaccard'):
    return rust.jaccard(df, col_a, col_b, name)

def sorensen_dice(df, col_a, col_b, name='sorensen_dice'):
    return rust.sorensen_dice(df, col_a, col_b, name)

__all__ = [
    "levenshtein",
    "jaro",
    "jaro_winkler",
    "jaccard",
    "sorensen_dice",
]
